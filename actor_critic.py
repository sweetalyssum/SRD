...
def build_model(carpe_diem):
    ...
    all_policy_variable = policy_variable + lstm_variables
    ...
    value_first = tf.reshape(tf.matmul(q_state, W), [1, 1]) + W_b # [1,1]
    ...
    value_first_next = tf.reshape(tf.matmul(states_first_next, W), [1, 1]) + W_b  # [1,1]
    eta_first = R_t + value_first_next - value_first
    
    with tf.variable_scope('loss_first_policy'):
        loss_first_policy = - tf.matmul(eta_first, tf.matmul(action, tf.reshape(tf.log(tf.clip_by_value(prob_first, 1e-30, 1.0)), [-1, 1])))
        optimizer_first_policy = tf.train.AdagradOptimizer(carpe_diem.learning_rate).minimize(loss_first_policy, var_list=all_policy_variable)

    with tf.variable_scope('loss_first_value'):
        optimizer_first_value = tf.train.AdagradOptimizer(carpe_diem.learning_rate)
        gradient_first_value = optimizer_first_value.compute_gradients(value_first, var_list=value_variable)
        new_gradient_first_value = [(-eta_first*g, v) for (g,v) in gradient_first_value]
        update_first_value = optimizer_first_value.apply_gradients(new_gradient_first_value)
    ...
    value = tf.reshape(tf.matmul(states, W), [1, 1]) + W_b # [1,1]
    ...
    value_next = tf.reshape(tf.matmul(states_next, W), [1, 1]) + W_b # [1,1]
    eta = R_t + value_next - value
    
    with tf.variable_scope('loss_policy'):
        loss_policy = - tf.matmul(eta, tf.matmul(action, tf.reshape(tf.log(tf.clip_by_value(prob, 1e-30, 1.0)), [-1, 1])))
        optimizer_policy = tf.train.AdagradOptimizer(carpe_diem.learning_rate).minimize(loss_policy, var_list=all_policy_variable)
       
    with tf.variable_scope('loss_value'): 
        optimizer_value = tf.train.AdagradOptimizer(carpe_diem.learning_rate)
        gradient_value = optimizer_value.compute_gradients(value, var_list=value_variable)
        new_gradient_value = [(-eta*g, v) for (g,v) in gradient_value]
        update_value = optimizer_value.apply_gradients(new_gradient_value)

    return optimizer_policy, update_value, prob, optimizer_first_policy, update_first_value, prob_first
...
opt, opt_value, doc_pred, opt_first, opt_first_value, doc_pred_first = build_model(carpe_diem)
...
while True:
    for query_id in listKeys:
        ...
        for id_i in range(carpe_diem.lenTrainPermutation):
            ...
            if id_i == 0:
                sess.run([opt_first, opt_first_value], feed_dict={input_query: [q], query_selected_next: listSelectedSet_repr[:1], candidate: candidate_set[id_i], action: candidate_set_id[id_i], R_t: [[r_t]]})
            else:
                sess.run([opt, opt_value], feed_dict={input_query: [q], query_selected: listSelectedSet_repr[:id_i], query_selected_next: listSelectedSet_repr[:id_i+1], candidate: candidate_set[id_i], action: candidate_set_id[id_i], R_t: [[r_t]]})
