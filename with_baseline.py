...
def build_model(carpe_diem):
    ...
    with tf.variable_scope('loss_first_policy'):
        loss_first_policy = - tf.matmul(G_t-value_first, tf.matmul(action, tf.reshape(tf.log(tf.clip_by_value(prob_first, 1e-30, 1.0)), [-1, 1])))
        optimizer_first_policy = tf.train.AdagradOptimizer(carpe_diem.learning_rate).minimize(loss_first_policy, var_list=policy_variable)
    ...
    with tf.variable_scope('loss_first_value'):
        ...
        new_gradient_first_value = [(-(G_t-value_first)*g, v) for (g,v) in gradient_first_value]
    ...
    with tf.variable_scope('loss_policy'):
        loss_policy = - tf.matmul(G_t-value, tf.matmul(action, tf.reshape(tf.log(tf.clip_by_value(prob, 1e-30, 1.0)), [-1, 1])))
    ...
    with tf.variable_scope('loss_value'):
        ...
        new_gradient_value = [(-(G_t-value)*g, v) for (g,v) in gradient_value]
...
while True:
    for query_id in listKeys:
        ...
        for id_i in range(carpe_diem.lenTrainPermutation):
            ...
            if id_i == 0:
                sess.run([opt_first, opt_first_value], feed_dict={input_query: [q], candidate: candidate_set[id_i], action: candidate_set_id[id_i], G_t: [[g_t]]})
            else:
                sess.run([opt, opt_value], feed_dict={input_query: [q], query_selected: listSelectedSet_repr[:id_i], candidate: candidate_set[id_i], action: candidate_set_id[id_i], G_t: [[g_t]]})
